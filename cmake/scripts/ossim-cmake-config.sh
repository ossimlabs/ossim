...
if [ -z "$BUILD_OSSIM_REGISTRATION_SOURCE" ]; then
  BUILD_OSSIM_REGISTRATION_SOURCE=OFF
fi
if [ -z "$OSSIM_AUTOREG_ROOT" ] && [ -d "$OSSIM_DEV_HOME/ossim-autoreg" ]; then
  export OSSIM_AUTOREG_ROOT="$OSSIM_DEV_HOME/ossim-autoreg"
fi
if [ -z "$OSSIM_REGISTRATION_SOURCE_ROOT" ] && [ -d "$OSSIM_DEV_HOME/ossim-registration-source" ]; then
  export OSSIM_REGISTRATION_SOURCE_ROOT="$OSSIM_DEV_HOME/ossim-registration-source"
fi
...
if [ "$BUILD_OSSIM_QT4" == "ON" ]; then
   if [ -z $QT_BINARY_DIR ]; then
      export QT_BINARY_DIR="/usr/local/bin"
   fi
fi
...
if [ -z $BUILD_MAXAR_APPS ] ; then
  export BUILD_MAXAR_APPS=ON
fi
...
-DBUILD_OSSIM_MPI_SUPPORT=$BUILD_OSSIM_MPI_SUPPORT \
-DBUILD_OSSIM_ISA=$BUILD_OSSIM_ISA \
-DBUILD_OSSIM_FPGA=$BUILD_OSSIM_FPGA \
-DBUILD_OSSIM_DEEPCORE=$BUILD_OSSIM_DEEPCORE \
...
-DKAKADU_INCLUDE_DIR=$KAKADU_INCLUDE_DIR \
...
"-DBUILD_OSSIM_REGISTRATION_SOURCE=$BUILD_OSSIM_REGISTRATION_SOURCE" \
"-DOSSIM_AUTOREG_ROOT=$OSSIM_AUTOREG_ROOT" \
"-DOSSIM_REGISTRATION_SOURCE_ROOT=$OSSIM_REGISTRATION_SOURCE_ROOT" \
-DBUILD_OSSIM_QT4=$BUILD_OSSIM_QT4 \
-DQT_BINARY_DIR=$QT_BINARY_DIR \
...
-DUSE_OSSIM_JSONCPP=$USE_OSSIM_JSONCPP \
-DBUILD_MAXAR_APPS=ON \
$CMAKE_DIR
