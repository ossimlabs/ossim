...
ossimProjection* ossimMapProjectionFactory::createProjection(const ossimString &name) const
{
   ...
   if(name == STATIC_TYPE_NAME(ossimBilinearMapProjection))
     return new ossimBilinearMapProjection;
  return NULL;
}
...
void ossimMapProjectionFactory::getTypeNameList(std::vector<ossimString>& typeList)const
{
   ...
   typeList.push_back(ossimString(STATIC_TYPE_NAME(ossimBilinearMapProjection)));
