...
// Description:
//
// Class definition of ossimU9ImageData.  Specialized image data object for
// unsigned short data with an 9 bit depth.
//
// NOTE:  This object is optimized for unsigned short data and assumes the
//        following:  null pixel value  = 0.0
//                    min  pixel value  = 1.0
//                    max  pixel value  = 511.0  (2^9 - 1)
//
...
RTTI_DEF1(ossimU9ImageData, "ossimU9ImageData", ossimImageData)

const ossimNormalizedU9RemapTable ossimU9ImageData::m_remapTable;
   
ossimU9ImageData::ossimU9ImageData()
   :
      ossimImageData()
{
   m_scalarType = OSSIM_UINT9;
}

ossimU9ImageData::ossimU9ImageData(ossimSource* source,
                                     ossim_uint32 bands)
   :
...
ossimU9ImageData::ossimU9ImageData(const ossimU9ImageData &rhs)
   :
      ossimImageData(rhs)
{}
...
ossimObject* ossimU9ImageData::dup()const
{
   return new ossimU9ImageData(*this);
}

ossimDataObjectStatus ossimU9ImageData::validate() const
{
...
void ossimU9ImageData::getNormalizedFloat(ossim_uint32 offset,
...
void ossimU9ImageData::setNormalizedFloat(ossim_uint32 offset,
                                           ossim_uint32 bandNumber,
                                           float inputValue)
{
   if( (getDataObjectStatus() != OSSIM_NULL) &&  isValidBand(bandNumber) )
...
void
ossimU9ImageData::convertToNormalizedFloat(ossimImageData* result)const
{
   if(!result)
   {
      return;
...
void ossimU9ImageData::convertToNormalizedDouble(ossimImageData* result)const
{
   if(!result)
   {
      return;
...
void ossimU9ImageData::unnormalizeInput(ossimImageData* normalizedInput)
{
...
double ossimU9ImageData::computeMeanSquaredError(double meanValue,
                                                  ossim_uint32 bandNumber)
{
...
double ossimU9ImageData::computeAverageBandValue(ossim_uint32 bandNumber)
{
   double result = 0.0;
   ossim_uint32 index = 0;
   ossim_uint32 validPixelCount=0;
...
void ossimU9ImageData::setValue(long x, long y, double color)
{
...
void ossimU9ImageData::fill(ossim_uint32 band, double value)
{
   void* s         = getBuf(band);
...
bool ossimU9ImageData::isNull(ossim_uint32 offset)const
{
...
void ossimU9ImageData::setNull(ossim_uint32 offset)
{
...
void ossimU9ImageData::copyTileToNormalizedBuffer(double* buf) const
{
   if (!buf)
   {
      ossimSetError(getClassName(),
                    ossimErrorCodes::OSSIM_ERROR,
                    "ossimU9ImageData::copyTileToNormalizedBuffer File %s line %d\nNull pointer passed to method!",
                    __FILE__,
                    __LINE__);
      return;
   }
   ...
   if(size > 0)
   {
      for(ossim_uint32 band = 0; band < getNumberOfBands(); ++band)
      {
         ...
         {
            d[index] = m_remapTable.normFromPix(static_cast<ossim_int32>(s[index]));
         }
...
void ossimU9ImageData::copyTileToNormalizedBuffer(ossim_uint32 band,
                                                   double* buf) const
{
   if (!buf)
   {
...
void ossimU9ImageData::copyNormalizedBufferToTile(double* buf)
{
   if (!buf)
   {
      ossimSetError(getClassName(),
...
void ossimU9ImageData::copyNormalizedBufferToTile(ossim_uint32 band,
                                                   double* buf)
{
   if (!buf)
   {
...
void ossimU9ImageData::copyTileToNormalizedBuffer(float* buf) const
{
   if (!buf)
   {
      ossimSetError(getClassName(),
...
void ossimU9ImageData::copyNormalizedBufferToTile(float* buf)
{
   if (!buf)
   {
      ossimSetError(getClassName(),
