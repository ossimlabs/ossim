 ...
 *    06/06/17          M. Thakkar    LSC-5523
 *                      Updated copy and equal operator to assign Geocent_algorithm
 */
...
Geocentric::Geocentric( const Geocentric &g )
{
  ...
  Geocent_algorithm = g.Geocent_algorithm;   // LSC-5523
}
...
/* Note: Variable names follow the notation used in Toms, Feb 1996 */
  ...
  else
  {
     ...
     if (At_Pole == FALSE)
     {
      ...
      double Rnn = semiMajorAxis / (sqrt(1.0e0 - Geocent_e2 * Sin2_Lat));
      double X1 = (Rnn + height) * Cos_Lat * cos(longitude);
      double Y1 = (Rnn + height) * Cos_Lat * sin(longitude);
      double Z1 = ((Rnn * (1 - Geocent_e2)) + height) * Sin_Lat;

      if( ( (X-X1) * (X-X1) + (Y-Y1) * (Y-Y1) + (Z-Z1) * (Z-Z1) ) > 1 )
      {
         throw CoordinateConversionException( "Legacy geocentric-to-geodetic is not accurate at this height." );
      }
  }
